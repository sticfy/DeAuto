...
let databaseColum = [{ "name": "id", "type": "int" }, { "name": "banner_type", "type": "int" }, { "name": "from_date", "type": "date" }, { "name": "to_date", "type": "date" }, { "name": "image", "type": "varchar" }, { "name": "details", "type": "text" }, { "name": "status", "type": "int" }, { "name": "created_by", "type": "int" }, { "name": "updated_by", "type": "int" }, { "name": "created_at", "type": "datetime" }, { "name": "updated_at", "type": "datetime" }];
...
let getListWithLimit = async () => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getListWithLimit(), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getBannerListByTypeAndDate = async (type = 0, todayDate) => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getBannerListByTypeAndDate(), [type, todayDate], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
