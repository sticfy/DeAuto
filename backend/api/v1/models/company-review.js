...
{ "name": "rating", "type": "double" }, { "name": "opinion", "type": "text" }, { "name": "status", "type": "int" }, { "name": "created_by", "type": "int" }, { "name": "updated_by", "type": "int" }, { "name": "created_at", "type": "datetime" }, { "name": "updated_at", "type": "datetime" }];
let jsonColum = [];
...
let companyAverageReviewById = async (companyId = 0) => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.companyAverageReviewById(), [companyId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
