...
const queries = require('../queries/company-subscribed-package');
...
let databaseColum =  [{"name":"id","type":"int"},{"name":"package_id","type":"int"},{"name":"company_id","type":"int"},{"name":"total_available_services","type":"int"},{"name":"total_available_bookings","type":"int"},{"name":"expired_date","type":"date"},{"name":"details","type":"text"},{"name":"status","type":"int"},{"name":"created_by","type":"int"},{"name":"updated_by","type":"int"},{"name":"created_at","type":"datetime"},{"name":"updated_at","type":"datetime"}];
...
let addNewWithMultipleInfo = async (info = {}, packageSubscribeHistoryData = {}, invoiceInfo = undefined, invoiceItemInfo = undefined) => {
    return new Promise((resolve, reject) => {
        ...
        connectionDeAutoMYSQL.getConnection(function (err, conn) {
            ...
            conn.beginTransaction(async function (error) {
                ...
                let temppackageSubscribeHistoryDataInsertId = 0;

                if (!isEmpty(packageSubscribeHistoryData)) {

                    let temPackageSubscribeHistoryResult = await companySubscribedPackageHistoryModel.addNew(packageSubscribeHistoryData, conn);
                    ...
                    temppackageSubscribeHistoryDataInsertId = temPackageSubscribeHistoryResult.insertId;
                }

                if (!isEmpty(invoiceInfo)) {

                    invoiceInfo.related_info_id =  temppackageSubscribeHistoryDataInsertId;
                    
                    let tempInvoiceResult = await invoiceModel.addNew(invoiceInfo, conn);

                    if (tempInvoiceResult.affectedRows == undefined || tempInvoiceResult.affectedRows < 1) {
                        return conn.rollback(function () {
                            conn.release();
                            resolve(tempInvoiceResult);
                        });
                    }

        
                    invoiceItemInfo.invoice_id = tempInvoiceResult.insertId;


                    let tempInvoiceItemResult = await invoiceItemModel.addNew(invoiceItemInfo, conn);

                    if (tempInvoiceItemResult.affectedRows == undefined || tempInvoiceItemResult.affectedRows < 1) {
                        return conn.rollback(function () {
                            conn.release();
                            resolve(tempInvoiceItemResult);
                        });
                    }
                }
...
let updateWithMultipleInfo = async (info = { "id": 0, "data": {} }, packageSubscribeHistoryData = {}, invoiceInfo = undefined, invoiceItemInfo = undefined) => {
    return new Promise((resolve, reject) => {
...
let updateSmsEmailAvailability = async (type = "",sentValue = 0,companyId = 0,updatedTime) => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.updateSmsEmailAvailability(type), [sentValue,updatedTime,companyId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
