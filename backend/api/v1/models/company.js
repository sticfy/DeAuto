...
const queriesCompanyUser = require('../queries/company-user');
...
let disableById = async (updatedBy, updatedAt, profileId, conn) => {
    let connection = connectionDeAutoMYSQL;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.disableById(), [updatedBy, updatedAt, profileId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let enableById = async (updatedBy, updatedAt, profileId, conn) => {
    let connection = connectionDeAutoMYSQL;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.enableById(), [updatedBy, updatedAt, profileId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let deleteById = async (updatedBy, updatedAt, profileId, conn) => {
    let connection = connectionDeAutoMYSQL;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.deleteById(), [updatedBy, updatedAt, profileId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let updateById = async (profileId, userId, profileData = {}, userData = {}) => {
        ...
        connectionDeAutoMYSQL.getConnection(function (err, conn) {
            ...
            conn.beginTransaction(async function (error) {
                ...
                if (!isEmpty(userData)) {
                    ...
                    conn.query(queriesCompanyUser.updateById(userData), [...dataParameter2, userId], async (error, result, fields) => {
                ...
                conn.query(queries.updateById(profileData), [...dataParameter, profileId], (error, result, fields) => {
...
let updateCompanyDataById = async (id = 0, updateData = {}, conn = undefined) => {
    ...
    return new Promise((resolve, reject) => {
        connection.query(queries.updateCompanyDataById(updateData), [...dataParameterUpdateData, id], (error, result, fields) => {
            if (error) reject(error);
            else resolve(result);
        });
    });
...
let updateCompanyAndImageById = async (companyId = 0, updateData = {}, imageArray = []) => {
    return new Promise((resolve, reject) => {
        ...
        connectionDeAutoMYSQL.getConnection(function (err, conn) {
            ...
            conn.beginTransaction(async function (error) {
                ...
                let result = await updateCompanyDataById(companyId, updateData, conn);
                ...
                if (imageArray.length > 0) {
                    ...
                    for (let index = 0; index < imageArray.length; index++) {
                        ...
                        imageAdd = await companyImageModel.addNew(element, conn);

                        if (imageAdd.affectedRows == undefined || imageAdd.affectedRows < 1) {
                            return conn.rollback(function () {
                                conn.release();
                                resolve(imageAdd);
                            });
