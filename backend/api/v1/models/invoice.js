...
const queries = require('../queries/invoice');
const invoiceItemModel = require('../models/invoice-item');


let databaseColum = [{"name":"id","type":"int"},{"name":"user_id","type":"int"},{"name":"company_id","type":"int"},{"name":"transaction_id","type":"int"},{"name":"inv_no","type":"varchar"},{"name":"type","type":"int"},{"name":"sub_total_price","type":"double"},{"name":"discount_price","type":"double"},{"name":"total_price","type":"double"},{"name":"related_info_id","type":"int"},{"name":"status","type":"int"},{"name":"created_by","type":"int"},{"name":"updated_by","type":"int"},{"name":"created_at","type":"datetime"},{"name":"updated_at","type":"datetime"}];
...
let addNewWithMultipleInfo = async (info, invoiceItemInfo = undefined ) => {
    return new Promise((resolve, reject) => {
        ...
        connectionDeAutoMYSQL.getConnection(function (err, conn) {
            ...
            conn.beginTransaction(async function (error) {
                ...
                let tempResult = await invoiceItemModel.addNew(invoiceItemInfo, conn);

                if (tempResult.affectedRows == undefined || tempResult.affectedRows < 1) {
                    return conn.rollback(function () {
                        conn.release();
                        resolve(tempResult);
                    });
