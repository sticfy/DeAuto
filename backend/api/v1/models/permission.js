...
const queries = require('../queries/permission');
const isEmpty = require("is-empty");

let databaseColum = [{"name":"id","type":"int"},{"name":"title","type":"varchar"},{"name":"key_name","type":"varchar"},{"name":"access_user","type":"int"},{"name":"api_version","type":"varchar"},{"name":"file_name","type":"varchar"},{"name":"url","type":"text"},{"name":"method","type":"varchar"},{"name":"details","type":"text"},{"name":"status","type":"int"},{"name":"created_by","type":"int"},{"name":"updated_by","type":"int"},{"name":"created_at","type":"datetime"},{"name":"updated_at","type":"datetime"}];
let jsonColum = [];
...
let getPermissionList = async () => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getPermissionList(), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getPermissionListByIDsAndWhere = async (permissionIdList = [], where = {}) => {

    let keysOfUpdateData = Object.keys(where);
    let dataParameterUpdateData = [];

    for (let index = 0; index < keysOfUpdateData.length; index++) {
        dataParameterUpdateData.push(where[keysOfUpdateData[index]]);
    }

    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getPermissionListByIDsAndWhere(permissionIdList, ...dataParameterUpdateData), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getPermissionListByIDAndAccessUserNotAdmin = async (permissionList = []) => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getPermissionListByIDAndAccessUserNotAdmin(permissionList), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getNotAssignPermissionList = async () => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getNotAssignPermissionList(), (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getByKeyName = async (keyName = "") => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getByKeyName(), [keyName], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getPermissionDetailsById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionDeAutoMYSQL.query(queries.getPermissionDetailsById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
