...
let table_name = "deautodb_modules";
let table_module_permissions = "deautodb_module_permissions";
let table_permissions = "deautodb_permissions";

let getList = () => {
    return `SELECT id, title, status FROM ${table_name}  where status != 0 order by title`;
}

let getActiveList = () => {
    return `SELECT id, title, status FROM ${table_name}  where status = 1 `;
}

let getByTitle = () => {
    return `SELECT id, title, status FROM ${table_name} where  title = ? and status != 0`;
}

let getById = () => {
    return `SELECT id, title, status FROM ${table_name} where  id = ? and status != 0`;
}

let getModuleByIdWithoutStatus = () => {
    return `SELECT id, title, status FROM ${table_name} where  id = ? and status != 0`;
}
