...
let table_name = "deautodb_module_permissions";
let table_permissions = "deautodb_permissions";

let getModulePermissionDetailsByModuleId = () => {
    return `SELECT ${table_name}.id as module_permission_id,${table_name}.module_id as module_id,${table_permissions}.id as permission_id,
     ${table_permissions}.title as permission_title,${table_permissions}.key_name,
     ${table_name}.status FROM ${table_name}
    join ${table_permissions} on (${table_permissions}.id = ${table_name}.permission_id)
    where  ${table_name}.module_id = ? and ${table_name}.status != 0`;
}

let getCommonPermissionListByCommonPermissionId = () => {
    return `SELECT * FROM ${table_permissions} where id in (SELECT permission_id FROM ${table_name} where status = 1  and module_id = 11)`;
}
