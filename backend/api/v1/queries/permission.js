...
let table_name = "deautodb_permissions";
let table_module_permissions_name = "deautodb_module_permissions";
let table_module__name = "deautodb_modules";

let getList = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name}  where status != 0 order by title `;
}
...
let getPermissionListByIDsAndWhere = (permissionIdList = [0], whereCondition = {}) => {

    let keys = Object.keys(whereCondition);

    let query = " SELECT id, title, key_name, access_user,  details, status FROM " + table_name + " where status = 1 and id IN (" + permissionIdList +") ";
...
let getPermissionListByIDAndAccessUserNotAdmin = (permissionIdList = [0]) => {
    let a = "SELECT * FROM "+ table_name +" where id in (SELECT permission_id FROM "+ table_module_permissions_name +" where status = 1) and id in (" + permissionIdList + ") and access_user != 1";
    return a;
    // console.log(a);
}

let getNotAssignPermissionList = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name} where status = 1 and access_user in (2,3) and id not in (SELECT permission_id FROM ${table_module_permissions_name}  where status != 0  and module_id in (SELECT id FROM ${table_module__name} where status != 0)) order by title`;
}

let getByTitle = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name} where  title = ? and status != 0`;
}

let getByKeyName = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name} where  key_name  = ? and status != 0`;
}

let getById = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name} where  id = ? and status != 0`;
}

let getPermissionDetailsById = () => {
    return `SELECT id, title, key_name, access_user,  details, status FROM ${table_name} where  id = ? and status = 1`;
}
...
module.exports = {
    getList,
    getPermissionList,
    getPermissionListByIDsAndWhere,
    getPermissionListByIDAndAccessUserNotAdmin,
    getNotAssignPermissionList,
    getById,
