...
let table_name = "deautodb_role_permissions";
let table_permissions = "deautodb_permissions";

let getRolePermissionDetailsByRoleId = () => {
    return `SELECT ${table_name}.id as role_permission_id,${table_name}.role_id as role_id,${table_permissions}.id as permission_id,
    ...
    where  ${table_name}.role_id = ? and ${table_name}.status != 0`;
}

let getCommonPermissionListByCommonPermissionId = () => {
    return `SELECT * FROM ${table_permissions} where id in (SELECT permission_id FROM ${table_name} where status = 1  and role_id = 11)`;
}
